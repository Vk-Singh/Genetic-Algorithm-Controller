...
class Controller:
    ...
    def calculate_fitness(self):
        '''
        print ("LEFT SPEED - {}".format(self.velocity_left))
        print ("Right Speed - {}".format(self.velocity_right) )
        '''
        ...
        #print ("Forward Fitness  = {}".format(forwardFitness))
        ### Define the fitness function to avoid collision
       #avoidCollisionFitness = ?
        
        avoidCollisionFitness = (1 - 1.7*(np.max(self.inputs[3:11]))**2)
        #print ("Distance = {}".format(np.max(self.inputs[3:11])))
        #print ("Avoid collision  = {}".format(avoidCollisionFitness))

        ### Define the fitness function to avoid spining behaviour
        #spinningFitness = ?

        if abs(self.velocity_left - self.velocity_right) > 1.8:
            spinningFitness += -1
        else:
            spinningFitness += 1
        ...
        combinedFitness = (0.5*forwardFitness + 0.75*avoidCollisionFitness + 0.75*spinningFitness)*10
